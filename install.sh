...
main() {
	...
	*) arch="${GOARCH}" ;;
	...
	TARBALL="${NAME}.${FORMAT}"
	TARBALL_URL="https://githubfast.com/twpayne/chezmoi/releases/download/${TAG}/${TARBALL}"
	http_download "${tmpdir}/${TARBALL}" "${TARBALL_URL}" || exit 1

	# download checksums
	CHECKSUMS="chezmoi_${VERSION}_checksums.txt"
	CHECKSUMS_URL="https://githubfast.com/twpayne/chezmoi/releases/download/${TAG}/${CHECKSUMS}"
